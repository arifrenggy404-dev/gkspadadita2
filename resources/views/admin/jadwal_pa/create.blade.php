...
<style>
    ...
    .page-header-lines { position: absolute; inset: 0; pointer-events: none; overflow: hidden; border-radius: 14px; }
    .page-header-lines::before {
        content: ''; position: absolute; top: -20px; left: 55%; width: 2px; height: 200%;
        background: linear-gradient(180deg, transparent, rgba(200,169,110,0.10), transparent);
        transform: rotate(20deg);
    }
    .page-header-lines::after {
        content: ''; position: absolute; top: -20px; left: 58%; width: 1px; height: 200%;
        background: linear-gradient(180deg, transparent, rgba(200,169,110,0.06), transparent);
        transform: rotate(20deg);
    }
    .page-header-cross {
        position: absolute; right: 24px; top: 50%; transform: translateY(-50%);
        font-size: 72px; color: rgba(200,169,110,0.09); line-height: 1;
    ...
    .page-header h4 { color: #fff; font-weight: 700; font-size: 1.25rem; margin-bottom: 4px; position: relative; z-index: 1; }
    .page-header p  { color: rgba(255,255,255,0.5); font-size: 13px; margin: 0; position: relative; z-index: 1; }
    ...
    .field-section { padding-bottom: 24px; margin-bottom: 24px; border-bottom: 1px solid #F3F4F6; }
    .field-section:last-of-type { border-bottom: none; padding-bottom: 0; margin-bottom: 0; }
    .section-label {
        font-size: 0.68rem; font-weight: 700; letter-spacing: 0.09em; text-transform: uppercase;
        color: #1a2744; margin-bottom: 16px; display: flex; align-items: center; gap: 8px;
    }
    ...
    /* Tombol trigger — tampil seperti select */
    .cs-trigger {
        width: 100%;
    ...
    .btn-back {
        ...
        font-size: 13px; font-weight: 500; color: #6B7280; background: #F9FAFB;
    ...
    .form-actions {
        ...
        padding-top: 28px; border-top: 1px solid #F3F4F6; margin-top: 28px; flex-wrap: wrap;
    }
    .btn-save {
        background: linear-gradient(135deg, #1a2744, #2a3f6f);
        border: none; color: #fff; font-weight: 600; border-radius: 9px; padding: 10px 28px;
        display: inline-flex; align-items: center; gap: 7px; transition: opacity 0.15s, transform 0.15s;
    }
    ...
    .btn-cancel {
        ...
        display: inline-flex; align-items: center; gap: 7px; transition: border-color 0.15s, color 0.15s;
    }
...
<div class="page-wrapper">
    ...
    <div class="page-header mb-4">
        ...
        <h4>Tambah Jadwal PA Baru</h4>
        <p>Isi data lengkap jadwal Pendalaman Alkitab di bawah ini.</p>
    </div>
    ...
    <div class="form-card">
        <div class="form-card-body">
            <form action="{{ route('jadwal_pa.store') }}" method="POST">
                @csrf
                    ...
                    <div>
                        ...
                        <div class="input-group-icon">
                            ...
                            <div class="cs-wrapper">
                                <button type="button"
                                        class="cs-trigger {{ old('nama') ? '' : 'placeholder' }} {{ $errors->has('nama') ? 'is-invalid' : '' }}"
                                        id="cs-nama">
                                    <span class="cs-label">{{ old('nama') ?: '-- Pilih Nama Jemaat --' }}</span>
                                    <i class="bi bi-chevron-down cs-chevron"></i>
                                </button>
                                <div class="cs-dropdown" id="cs-nama-dropdown">
                                    <div class="cs-search-box">
                                        <input type="text" placeholder="Cari nama jemaat..."
                                               id="cs-nama-search" autocomplete="off">
                                    </div>
                                    <div class="cs-options" id="cs-nama-options">
                                        @foreach($jemaat as $j)
                                            <div class="cs-option {{ old('nama') == $j->nama_jemaat ? 'selected' : '' }}"
                                                 data-value="{{ $j->nama_jemaat }}">
                                                {{ $j->nama_jemaat }}
                                            </div>
                                        @endforeach
                                    </div>
                                    <div class="cs-no-results" id="cs-nama-noresult">Tidak ditemukan</div>
                                </div>
                            </div>
                        </div>
                        @error('nama')
                ...
                <div class="field-section">
                    ...
                    <div class="row g-3">
                        <div class="col-12 col-md-6">
                            <label class="form-label fw-semibold small">Pelayan Firman</label>

                            <input type="hidden" name="pelayan" id="pelayan-value"
                                   value="{{ old('pelayan') }}" required>
                        ...
                        <div class="col-12 col-md-6">
                            <label class="form-label fw-semibold small">Pendamping</label>

                            <input type="hidden" name="pendamping" id="pendamping-value"
                            ...
                            <div class="input-group-icon">
                                ...
                                <div class="cs-wrapper">
                                    <button type="button"
                                            class="cs-trigger {{ old('pendamping') ? '' : 'placeholder' }} {{ $errors->has('pendamping') ? 'is-invalid' : '' }}"
                                            id="cs-pendamping">
                                        <span class="cs-label">{{ old('pendamping') ?: '-- Pilih Pendamping --' }}</span>
                                        <i class="bi bi-chevron-down cs-chevron"></i>
                                    </button>
                                    <div class="cs-dropdown" id="cs-pendamping-dropdown">
                                        <div class="cs-search-box">
                                            <input type="text" placeholder="Cari pendamping..."
                                                   id="cs-pendamping-search" autocomplete="off">
                                        </div>
                                        <div class="cs-options" id="cs-pendamping-options">
                                            @foreach($pelayanans as $p)
                                                <div class="cs-option {{ old('pendamping') == $p->nama ? 'selected' : '' }}"
                                                     data-value="{{ $p->nama }}">
                                                    {{ $p->nama }}
                                                </div>
                                            @endforeach
                                        </div>
                                        <div class="cs-no-results" id="cs-pendamping-noresult">Tidak ditemukan</div>
                                    </div>
                                </div>
                            </div>
                            @error('pendamping')
...
<script>
/**
 * Custom select dengan search di dalam dropdown.
 * Tidak pakai library apapun — zero conflict.
 *
 * @param {string} triggerId   — id tombol trigger
 * @param {string} dropdownId  — id panel dropdown
 * @param {string} searchId    — id input pencarian
 * @param {string} optionsId   — id container opsi
 * @param {string} noResultId  — id elemen "tidak ditemukan"
 * @param {string} hiddenId    — id hidden input (dikirim ke server)
 */
function makeCustomSelect(triggerId, dropdownId, searchId, optionsId, noResultId, hiddenId) {
    ...
    // Filter opsi berdasarkan kata kunci
...
// Inisialisasi ketiga select
makeCustomSelect(
    'cs-nama', 'cs-nama-dropdown', 'cs-nama-search',
    'cs-nama-options', 'cs-nama-noresult', 'nama-value'
);
makeCustomSelect(
...
makeCustomSelect(
    'cs-pendamping', 'cs-pendamping-dropdown', 'cs-pendamping-search',
    'cs-pendamping-options', 'cs-pendamping-noresult', 'pendamping-value'
);
</script>
@endsection