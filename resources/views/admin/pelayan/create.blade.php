...
@section('title', 'Tambah Data Pelayan')
...
<style>
    ...
    /* Icon box dalam status card */
    .status-option label .status-icon {
        width: 40px; height: 40px; border-radius: 10px;
        display: flex; align-items: center; justify-content: center;
        font-size: 1.25rem; transition: all 0.15s;
    }
    .status-option.opt-pendeta label .status-icon { background: #ede9fe; color: #7c3aed; }
    .status-option.opt-vic     label .status-icon { background: #e0f2fe; color: #0284c7; }
    .status-option.opt-majelis label .status-icon { background: #dcfce7; color: #16a34a; }
    ...
    .status-option.opt-pendeta input[type="radio"]:checked + label {
        border-color: #7c3aed; background: #faf8ff; color: #7c3aed;
    }
    .status-option.opt-pendeta input[type="radio"]:checked + label::after { background: #7c3aed; }
    .status-option.opt-pendeta input[type="radio"]:checked + label .status-icon {
        background: #7c3aed; color: #fff;
    }

    .status-option.opt-vic input[type="radio"]:checked + label {
        border-color: #0284c7; background: #f0f9ff; color: #0284c7;
    }
    .status-option.opt-vic input[type="radio"]:checked + label::after { background: #0284c7; }
    .status-option.opt-vic input[type="radio"]:checked + label .status-icon {
        background: #0284c7; color: #fff;
    }

    .status-option.opt-majelis input[type="radio"]:checked + label {
        border-color: #16a34a; background: #f0fdf4; color: #16a34a;
    }
    .status-option.opt-majelis input[type="radio"]:checked + label::after { background: #16a34a; }
    .status-option.opt-majelis input[type="radio"]:checked + label .status-icon {
...
<div class="page-wrapper">
    ...
    <div class="page-header mb-4">
        ...
        <h4>Tambah Pelayan Baru</h4>
        <p>Isi data lengkap pelayan baru di bawah ini.</p>
    </div>
            ...
            <form action="{{ route('pelayan.store') }}" method="POST">
                ...
                <div class="field-section">
                    ...
                    <div class="status-grid">
                        <div class="status-option opt-pendeta">
                            <input type="radio" id="pendeta" name="status" value="Pendeta"
                                {{ old('status') == 'Pendeta' ? 'checked' : '' }} required>
                            <label for="pendeta">
                                <div class="status-icon"><i class="bi bi-book-fill"></i></div>
                                Pendeta
                            </label>
                        </div>
                        <div class="status-option opt-vic">
                            <input type="radio" id="vic" name="status" value="Vic"
                                {{ old('status') == 'Vic' ? 'checked' : '' }}>
                            <label for="vic">
                                <div class="status-icon"><i class="bi bi-person-badge-fill"></i></div>
                                Vic
                            </label>
                        </div>
                        <div class="status-option opt-majelis">
                            <input type="radio" id="majelis" name="status" value="Majelis"
                                {{ old('status') == 'Majelis' ? 'checked' : '' }}>
                            <label for="majelis">
