...
<style>
    ...
    .btn-reset {
        border-radius: 9px; padding: 8px 14px; font-size: 0.875rem;
    }
    ...
    .table-pelayan { margin: 0; }
    .table-pelayan thead th {
        background: #1a2744;
    ...
    .table-pelayan thead th:first-child { padding-left: 20px; }
    .table-pelayan thead th:last-child  { padding-right: 20px; }

    .table-pelayan tbody tr {
        border-bottom: 1px solid #F3F4F6;
        transition: background 0.12s;
    }
    .table-pelayan tbody tr:last-child { border-bottom: none; }
    .table-pelayan tbody tr:hover { background: #f8f7ff; }
    .table-pelayan tbody td {
        padding: 12px 14px; vertical-align: middle;
        font-size: 0.875rem; color: #374151;
    }
    .table-pelayan tbody td:first-child { padding-left: 20px; }
    .table-pelayan tbody td:last-child  { padding-right: 20px; }
    ...
    .status-pendeta { background: #d1fae5; color: #065f46; }
    .status-vic     { background: #fef3c7; color: #92400e; }
    .status-majelis { background: #fee2e2; color: #991b1b; }
    ...
    @media print {
        .sidebar, .navbar, .toolbar, .btn-aksi,
        .page-header, .header-actions { display: none !important; }
        .table-pelayan thead th:last-child,
        .table-pelayan tbody td:last-child { display: none !important; }
        .data-card { box-shadow: none !important; border: none !important; }
        .data-card::before { display: none; }
        .table-pelayan th, .table-pelayan td {
...
<div class="container-fluid py-3">
    ...
    {{-- Filter Pencarian --}}
    <div class="toolbar">
        <form action="{{ route('pelayan.index') }}" method="GET" class="search-box">
            <input type="text" name="search" class="form-control" placeholder="Cari nama pelayan..." value="{{ request('search') }}">
            <button type="submit" class="btn-search">
                <i class="bi bi-search"></i>
            </button>
            @if(request('search'))
                <a href="{{ route('pelayan.index') }}" class="btn btn-outline-danger btn-reset">Reset</a>
            @endif
        </form>
    </div>
    ...
    <div class="data-card">
        <div class="table-responsive">
            <table class="table table-hover table-pelayan w-100">
                <thead>
                    <tr>
                ...
                <tbody>
                    @forelse($Pelayan as $index => $item)
                    <tr>
                        <td><div class="no-badge">{{ $index + 1 }}</div></td>
                        <td class="nama-cell">{{ $item->nama }}</td>
                        <td>
                            @if($item->status == 'Pendeta')
                                <span class="status-badge status-pendeta">Pendeta</span>
                            @elseif($item->status == 'Vic')
                                <span class="status-badge status-vic">Vic</span>
                            @else
                                <span class="status-badge status-majelis">Majelis</span>
                            @endif
                        </td>
                        <td>
...
@endif
                        ...
                        <td class="text-center">
                            <div class="d-flex justify-content-center gap-2">
                                <a href="{{ route('pelayan.edit', $item->id) }}" class="btn-aksi btn-edit" title="Edit">
                                    <i class="bi bi-pencil-square"></i>
                                </a>
                                <form action="{{ route('pelayan.destroy', $item->id) }}" method="POST" class="m-0" onsubmit="return confirm('Yakin hapus?')">
                                    @csrf @method('DELETE')
                    ...
                    <tr>
                        <td colspan="7">
                            <div class="empty-state">
                                ...
                                <p>Data pelayan tidak ditemukan.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
